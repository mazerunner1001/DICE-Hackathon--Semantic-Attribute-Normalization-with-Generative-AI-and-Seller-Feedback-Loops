// src/App.jsx (robust version — safe optional chaining & fallback images)
import React, { useState, useEffect, useRef } from "react";
import axios from "axios";
import "./App.css";

/* Small inline SVG placeholder (data URI) to avoid external network calls */
const INLINE_PLACEHOLDER = `data:image/svg+xml;utf8,${encodeURIComponent(
  `<svg xmlns='http://www.w3.org/2000/svg' width='220' height='160'><rect width='100%' height='100%' fill='#efe6fb'/><text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' font-family='Arial' font-size='14' fill='#6d2fb0'>Placeholder</text></svg>`
)}`;
...
/* Carousel uses inline placeholder to avoid external fetch issues */
      ...
      {tiles.map((t,i)=>(
        <div className="carousel-card" key={i}>
          <div className="carousel-art">
            ...
            <img alt={t.title} src={INLINE_PLACEHOLDER} />
          </div>
...
export default function App() {
  ...
  async function handleAnalyze() {
    if (!title && !desc) { setErrorMsg("Add a title or description for better text signals"); } else { setErrorMsg(""); }
    ...
    try {
      ...
      setResp(r.data || null);
      setCorrectionSelection((r.data && r.data.final_category && r.data.final_category.value) ? r.data.final_category.value : "");
    } catch (err) {
            ...
            {resp && (
              <div className="card result">
                <div className="result-head">
                  <div>
                    <h3 className="final-cat">{resp?.final_category?.value ?? "Unknown"}</h3>
                    <div className="meta">
                      Confidence: {((resp?.final_category?.confidence ?? resp?.final_category?.text_score ?? 0)*100).toFixed(1)}% • Time: {resp?.timing_sec ?? "N/A"}s • Pipeline: {resp?.pipeline ?? pipeline}
                    </div>
                  </div>
                  <div>
                    ...
                    <div key={i} className="img-result-card">
                      <div className="img-left">
                        <div className="img-stack">
                          {/* choose preview or mask or inline placeholder */}
                          <img src={previews[i] ?? (img?.mask_overlay ?? INLINE_PLACEHOLDER)} alt={"orig-"+i} className="orig" />
                      ...
                      <div className="img-right">
                        <div className="row">
                          <div className="col">
                            ...
                            {(img?.category_probs ?? img?.category_scores ?? []).slice(0,4).map((p, k)=>(
                          ...
                          <div className="col">
                            ...
                            <div className="color-chip-row">
                              {(img?.colors ?? []).length > 0 ? (img.colors.map((c, idx)=>(
                        ...
                        <div className="row heur">
                          ...
                          <div className="heur-list">
                            {img?.heuristics ? Object.entries(img.heuristics).slice(0,4).map(([k,v], idx)=>(<div key={idx} className="heur-item">{k}: {(v*100).toFixed(0)}%</div>)) : <div className="muted">No heuristics</div>}
                          </div>
                        </div>
